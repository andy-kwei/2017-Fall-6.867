...
#     plt.title("MSE vs. Regression Dimensionality for Polynomial Basis")
#     plt.legend()
#     plt.show()
#     
    #print(type(theta))
    ...
    Ms = [0,1,2,3,4,5,6,7]
    ls = [0,.5,1,1.5,2,2.5,3,3.5,4]
    #Remove outlier
    BData_X = np.matrix(np.delete(BData_X,7)).T
    BData_Y = np.matrix(np.delete(BData_Y,7)).T
    print(BData_X)
    print(BData_Y)
#     plt.scatter(AData_X,AData_Y)
#     plt.title("Data Points in AData")
#     axes = plt.gca()
#     axes.set_xlim([-4,4])
#     axes.set_ylim([-4,4])
#     plt.show()
#     
#     plt.scatter(BData_X,BData_Y)
#     plt.title("Data Points in BData")
#     axes = plt.gca()
#     axes.set_xlim([-4,4])
#     axes.set_ylim([-4,4])
#     plt.show()
    ...
    train_set_X,train_set_Y = BData_X,BData_Y
    test_set_X,test_set_Y = AData_X,AData_Y
    print(test_set_X)
    ...
    #print("MSEs: ",MSEs)
    bestModels = np.argmin(MSEs,axis=1)
    print(bestModels)
    
    test_errors = [0]*len(ls)
    MSE_errors = [0]*len(ls)
    for i in range(len(ls)):
        myTheta = thetas[i][bestModels[i]]
        myMSE = MSEs[i][bestModels[i]]
        MSE_errors[i] = myMSE
        test_error = MSE(predictRidge(test_set_X,myTheta),test_set_Y)
        test_errors[i] = test_error[0].item()
