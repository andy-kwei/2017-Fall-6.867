...
def plotDecisionBoundary(X, Y, scoreFn, values, title = "", colors='green', cmap=pl.cm.cool):
    ...
    pl.figure()
    CS = pl.contour(xx, yy, zz, values, colors = colors, linestyles = 'solid', linewidths = 2)
    pl.clabel(CS, fontsize=9, inline=1)
    # Plot the training points
    pl.scatter(X[:, 0], X[:, 1], c=(1.-Y), s=50, cmap=cmap)
    pl.title(title)
    pl.axis('tight')
